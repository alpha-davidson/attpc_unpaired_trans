        ...
        with h5py.File(self.path, mode='r') as f:
            for pc, pc_id, cate_name in _enumerate_pointclouds(f):
                ...
                if self.scale_mode == 'global_unit':
                    ...
                    shift = torch.zeros([1, 4]) #changed from 1,3
                    scale = torch.ones([1, 1])

                
                # pc = (pc - shift) / scale
                    scaled_pc = torch.zeros_like(pc)
                    for i in range(pc[:,0].size(0)):
                        scaled_pc[i,0] = pc[i,0] / 250
                        scaled_pc[i,1] = pc[i,1] / 250
                        scaled_pc[i,2] = pc[i,2] / 1000
                        scaled_pc[i,3] = pc[i,3] / 2500
                pc = scaled_pc
