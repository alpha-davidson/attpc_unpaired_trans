...
def unit_cube_grid_point_cloud(resolution, clip_sphere=False):
    ...
    """
    grid = np.ndarray((resolution, resolution, resolution, 3), np.float32) #here??
    ...
    for i in range(resolution):
        for j in range(resolution):
            for k in range(resolution):
                ...
                # grid[i, j, k, 3] = k * spacing - 0.5

    if clip_sphere:
        grid = grid.reshape(-1, 4) #changed here from 3 to 4
...
def entropy_of_occupancy_grid(
    ...
    grid_coordinates = grid_coordinates.reshape(-1, 4) #here too changed from 3 to 4
...
if __name__ == '__main__':
    a = torch.randn([16, 2048, 4]).cuda() # changed from 3 to 4
    b = torch.randn([16, 2048, 4]).cuda() # changed from 3 to 4
